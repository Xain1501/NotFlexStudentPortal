...
export default function FacultyAttendance() {
  ...
  const [marked_at, setMarkedAt] = useState(new Date().toISOString().slice(0,19));
  ...
  function viewRecord(record) {
    const names = faculty.filter(f => record.present.includes(f.id)).map(f => f.name);
    alert(
    ...
    <div className="container admin-main">
        ...
        <div className="card-body">
          ...
          <div className="d-flex flex-wrap justify-content-center align-items-end attendance-controls mb-3">
            <div className="form-group mx-2 text-center">
              <label>Date</label>
              <input type="date" className="form-control" value={attendance_date} onChange={e => setAttendanceDate(e.target.value)} />
            </div>
            ...
            <div className="form-group mx-2 text-center">
              ...
              <input className="form-control" value={marked_by} onChange={e => setMarkedBy(e.target.value)} placeholder="Admin ID" />
            </div>
            <div className="form-group mx-2 text-center">
              <label>Marked At</label>
              <input className="form-control" type="datetime-local" value={marked_at} onChange={e => setMarkedAt(e.target.value)} />
            </div>
      ...
      <div className="card mt-4">
        <div className="card-body">
          ...
          {history.length === 0 ? (
            <div className="text-muted"><em>No records</em></div>
          ) : (
            <>
              <ul className="list-group">
                {pagedHistory.map(h => (
                  <li key={h.attendance_id} className="list-group-item d-flex justify-content-between align-items-center">
                    <div>
                      {h.attendance_date} — {h.session}
                      <span className="small ms-2 text-muted"> ({(h.present||[]).length} present)</span>
                      {h.marked_at &&
                        <span className="small ms-2 text-success">Marked at: {h.marked_at}</span>
                      }
                      {h.marked_by &&
                        <span className="small ms-2 text-info">By: {h.marked_by}</span>
                      }
                    </div>
                    <div>
                      <button className="btn btn-outline-primary btn-sm" onClick={() => viewRecord(h)}>View</button>
                    </div>
                  </li>
                ))}
              </ul>
              ...
              <div className="d-flex justify-content-between align-items-center mt-3">
                ...
                <div>
                  <button className="btn btn-outline-secondary btn-sm me-2" disabled={page<=1} onClick={()=>setPage(p=>p-1)}>Prev</button>
