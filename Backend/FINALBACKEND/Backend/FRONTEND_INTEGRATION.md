...
3. Run preflight checks and start the backend server from project root:
  - `python tools/preflight.py` (optional `--seed` flag to run seed script)
  - `python main.py`
   
  Tip: If you prefer a Powershell-friendly setup or the server fails to start directly, follow these exact steps:
  ...
  # (Optional) set environment variables in PowerShell
  $env:DB_HOST = 'localhost';
  $env:DB_USER = 'portal_app';
  $env:DB_PASSWORD = 'Portal@1234!';
  ...
  python .\tools\preflight.py --seed

  # Start the backend
  python main.py
  ```

  If `python main.py` doesn't start:
  - Ensure Python 3.10+ is installed and `python` points to the correct binary (`py -3 main.py` on some systems).
  - Check the venv activation step (you should see `(.venv)` prefix in your prompt).
  - Check `Backend/.env` is present and contains valid DB credentials.
  - Make sure MySQL server is running and accessible using the credentials provided.
...
-- DB_USER (default: `portal_app`)
-- DB_PASSWORD (default: `Portal@1234!` in example) — *replace in `.env`* for dev and production
