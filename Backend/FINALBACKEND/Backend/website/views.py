...
@views.route('/api/admin/courses/<int:course_id>', methods=['PUT'])
@token_required
def update_course_admin(current_user, course_id):
    """Update course (Admin only)"""
    if current_user['role'] != 'admin':
        return jsonify({'success': False, 'message': 'Access denied'}), 403
    try:
        data = request.get_json() or {}
        allowed_fields = ['course_code', 'course_name', 'credits', 'fee_per_credit', 'department_id']
        update_data = {k: data[k] for k in allowed_fields if k in data}
        if not update_data:
            return jsonify({'success': False, 'message': 'No valid fields to update'}), 400
        success = CourseModel.update_course(course_id, update_data)
        if success:
            return jsonify({'success': True, 'message': 'Course updated successfully'}), 200
        else:
            return jsonify({'success': False, 'message': 'Failed to update course'}), 400
    except Exception as e:
        return jsonify({'success': False, 'message': f'Error: {str(e)}'}), 500


@views.route('/api/admin/courses/<int:course_id>', methods=['DELETE'])
@token_required
def delete_course_admin(current_user, course_id):
    """Delete course (Admin only)"""
    if current_user['role'] != 'admin':
        return jsonify({'success': False, 'message': 'Access denied'}), 403
    try:
        success = CourseModel.delete_course(course_id)
        if success:
            return jsonify({'success': True, 'message': 'Course deleted successfully'}), 200
        else:
            return jsonify({'success': False, 'message': 'Failed to delete course'}), 400
    except Exception as e:
        return jsonify({'success': False, 'message': f'Error: {str(e)}'}), 500
